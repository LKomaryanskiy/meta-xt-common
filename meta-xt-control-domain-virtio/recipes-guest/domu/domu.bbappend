...
do_install:append() {
    ...
    install -m 0644 ${UNPACKDIR}/virtio-env.conf ${D}${sysconfdir}/systemd/system/domu.service.d
    ...
    if ${@bb.utils.contains('MACHINE_FEATURES', 'gsx', 'true', 'false', d)}; then
        install -m 0644 ${UNPACKDIR}/virtio-env-weston.conf ${D}${sysconfdir}/systemd/system/domu.service.d
    else
        install -m 0644 ${UNPACKDIR}/virtio-env-no-weston.conf ${D}${sysconfdir}/systemd/system/domu.service.d
    fi
}
